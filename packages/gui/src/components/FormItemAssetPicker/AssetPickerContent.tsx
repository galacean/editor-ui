...
export function AssetPickerContent<T extends BasicAssetType>(props: AssetPickerPopoverProps<T>) {
  ...
  const [displayMode, setDisplayMode] = useState<'list' | 'grid'>((window.localStorage.getItem(key) as any) || 'grid')
    ...
    <StyledAssetPickerContent>
      <StyledHeader gap="xs" wrap={false}>
        ...
        <Flex gap="xs" wrap={false} align="both">
          <ActionButton fancy variant="ghost" active={displayMode === 'grid'} onClick={handleChangeDisplayMode('grid')}>
            <IconLayoutGrid />
          </ActionButton>
          <ActionButton variant="ghost" fancy active={displayMode === 'list'} onClick={handleChangeDisplayMode('list')}>
            <IconLayoutList />
          </ActionButton>
        </Flex>
      </StyledHeader>
